...
contains
	subroutine cnt_band_structure()
		...
		double precision, dimension(2) :: k,E_tmp,E1_tmp,E2_tmp
		double precision, dimension(:), allocatable :: k_vec,min_energy
		double precision, dimension(:,:,:), allocatable :: E_k
		...
		allocate(Cc_k(currcnt%mu_min:currcnt%mu_max,currcnt%ik_min:currcnt%ik_max,2))
		allocate(Cv_k(currcnt%mu_min:currcnt%mu_max,currcnt%ik_min:currcnt%ik_max,2))
		allocate(min_loc(0:currcnt%mu_max))
		...
		! currcnt%iq_max=max(2*currcnt%ikr_high,currcnt%ik_max)		!the higher limit of the index in v_FT and esp_q
